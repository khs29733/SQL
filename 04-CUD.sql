...
--명시되지 않은 컬럼은 NULL로 입력 -> NOT NULL 제약 조건이 있는 필드는 오류
...
SELECT * FROM book;
VALUES(3, '홍길동전', sysdate, 3); --author_id는 author.author_id를 FK에 참조하고 있기 때문에
--제약조건 위반
...
--WHERE 절이 없는 DELETE는 모든 레코드를 삭제한다
DELETE FROM emp_123;
SELECT * FROM emp_123;
--DELETE는 Transaction의 대상 -> ROLLBACK가능
ROLLBACK;
SELECT * FROM emp_123;

--TRUNCATE : 테이블 비우기
TRUNCATE emp_123;
--주의 : TRUNCATE는 Transaction의 대상이 아니다 -> ROLLBACK불가
ROLLBACK
SELECT * FROM emp_123;
